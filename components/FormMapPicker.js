...
export default function FormMapPicker({ lat, lng, onChange, cityCenter }) {
  ...
  // Quando lat/lng mudam externamente (ex: edição de item existente), atualiza marcador e mapa
  useEffect(() => {
  ...
  // Quando cidade é selecionada: apenas PAN do mapa, sem mover o marcador de posição
  useEffect(() => {
    if (mapRef.current && cityCenter?.lat && cityCenter?.lng) {
      mapRef.current.flyTo([cityCenter.lat, cityCenter.lng], 12, { duration: 1.2 })
    }
  }, [cityCenter])
