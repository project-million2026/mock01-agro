...
export default function FieldMapEditor({ initialPolygon = [], existingFields = [], cityCenter = null, onChange, height = 460 }) {
  ...
  // Refocus map whenever initialPolygon prop changes (e.g. clicking a field in the table)
  useEffect(() => {
    if (mapRef.current && initialPolygon?.length >= 3) {
      const L = window.L
      if (!L) return
      const poly = L.polygon(initialPolygon)
      try { mapRef.current.fitBounds(poly.getBounds(), { padding: [20, 20], animate: true, duration: 1.0 }) } catch {}
    }
  }, [initialPolygon])
  ...
  useEffect(() => {
    if (mapRef.current && cityCenter && (!initialPolygon || initialPolygon.length < 3)) {
      mapRef.current.flyTo(cityCenter, 12, { duration: 1.5 })
    }
  }, [cityCenter, initialPolygon])
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const features = await geoUtils.parseGeoFile(file)
      if (!features || features.length === 0) throw new Error('Nenhum polígono encontrado no arquivo.')
      
      if (features.length === 1) {
        applyFeature(features[0])
      } else {
        setParsedFeatures(features)
      }
    } catch (err) {
      toast.error(err.message || 'Erro ao processar o arquivo de mapa.')
    } finally {
      setImporting(false)
  ...
  const applyFeature = (feature) => {
    ...
    const L = window.L
    if (drawnRef.current && L && mapRef.current) {
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <button 
          ...
          onClick={() => {
            ...
            if(isNaN(lat1) || isNaN(lng1) || isNaN(lat2) || isNaN(lng2)) return toast.error('Preencha todas as coordenadas');
          ...
          className="inline-flex h-9 items-center justify-center rounded-md bg-secondary border border-border px-4 py-2 text-sm font-medium hover:bg-secondary/80 flex-none w-full sm:w-auto mt-2 sm:mt-0"
        >
          Gerar Área
        </button>
        <div className="flex-none w-full sm:w-auto mt-2 sm:mt-0 ml-auto">
          <input type="file" accept=".zip,.kml,.geojson,.json" className="hidden" ref={fileInputRef} onChange={handleFileUpload} />
          <button
            type="button"
            onClick={() => fileInputRef.current?.click()}
            disabled={importing}
            title="Shapefile de drone (.zip), Google Earth (.kml) ou GeoJSON"
          ...
          >
            ...
            {importing ? 'Processando...' : 'Importar (Drone / Google Earth)'}
          </button>
        </div>
      </div>

      <p className="text-xs text-muted-foreground -mt-1">
        Delimite a área de <strong>3 formas</strong>: importe o levantamento de <strong>drone</strong> (Shapefile
        <code className="mx-1">.zip</code> — reprojetado automaticamente pelo <code>.prj</code>), um arquivo do
        <strong> Google Earth</strong> (<code className="mx-1">.kml</code>) ou GeoJSON; ou <strong>desenhe manualmente</strong> com
        a ferramenta de polígono no canto do mapa.
      </p>

      {parsedFeatures.length > 0 && (
        <div className="bg-card border border-border rounded-lg p-4 mb-2 shadow-lg max-h-64 overflow-y-auto">
          <div className="flex justify-between items-center mb-3">
            <h4 className="font-semibold text-sm">Selecione o polígono para importar:</h4>
            <button type="button" onClick={() => setParsedFeatures([])} className="text-xs text-muted-foreground hover:text-foreground">Cancelar</button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {parsedFeatures.map((feat, idx) => (
              <button
                key={idx}
                type="button"
                onClick={() => applyFeature(feat)}
                className="text-left text-sm p-3 rounded bg-secondary/50 hover:bg-secondary border border-border/50 hover:border-primary/50 transition-colors"
              >
                <div className="font-medium text-primary mb-1">
                  {feat.properties?.name || feat.properties?.Name || `Polígono ${idx + 1}`}
                </div>
                <div className="text-xs text-muted-foreground line-clamp-1">
                  {feat.properties?.description || 'Nenhuma descrição'}
                </div>
              </button>
            ))}
          </div>
