...
export default function DashboardPage({ onSelectFleet, setPage }) {
  ...
  useEffect(() => {
    ...
    // Polling apenas como reconciliação lenta; as atualizações chegam ao vivo via WebSocket.
    const i = setInterval(refresh, 60000)
    return () => clearInterval(i)
  }, [refresh])

  // Tempo real (SPRINT-07): posição e eventos recentes chegam via WebSocket, sem esperar o poll.
  useEffect(() => {
    return subscribeDashboard((msg) => {
      if (msg.type === 'position') {
        const p = msg.data
        setPositions((prev) => {
          const rest = prev.filter((x) => x.fleetNumber !== p.fleetNumber)
          return [...rest, p]
        })
        setRecent((prev) => [p, ...prev].slice(0, 50))
      }
    })
  }, [])
